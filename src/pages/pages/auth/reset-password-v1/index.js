...
const ResetPasswordV1 = () => {
  ...
  const handleOldPasswordChange = (event) => {
    setValues({ ...values, oldPassword: event.target.value }); // Update oldPassword
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (values.newPassword !== values.confirmNewPassword) {
      // Passwords do not match, display an error message or handle it as needed
      console.log('Passwords do not match');
      return;
    }

    // Passwords match, send Axios request to change password
    try {
      ...
      <Card sx={{ zIndex: 1 }}>
        <CardContent sx={{ p: theme => `${theme.spacing(12, 9, 7)} !important` }}>
          ...
          <FormControl sx={{ display: 'flex', mb: 4 }}>
              <InputLabel htmlFor='auth-reset-password-oldpassword'>Old Password</InputLabel>
              <OutlinedInput
                autoFocus
                label='Old Password'
                value={values.oldPassword}
                id='auth-reset-password-old-password'
